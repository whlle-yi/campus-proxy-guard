...
| 信号 | 性质 | 处理 |
| --- | --- | --- |
| **实时连接**(Clash/Mihomo API): 有访问学校域名的活动连接 | 实际违规 | 弹「代理访问学校网站警告」, 按 `school_site_disable_proxy` 自动关闭系统代理 |
| **系统代理接管**: 系统代理开启且学校域名未放入绕过列表 | 风险提示(不代表正在访问) | 仅窗口显示 + 每次会话弹一次说明, 不做任何处置 |

分级的原因: 系统代理开启只意味着"若访问学校网站会经过代理", 并不代表正在访问——
把它当违规会导致只要挂梯子就持续误报、系统代理无法正常使用。
